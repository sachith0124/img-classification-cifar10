...
class batch_norm_relu_layer(nn.Module):
    ...
    def __init__(self, num_features, eps=1e-5, momentum=0.997) -> None:
        ...
        self.bnorm1 = nn.BatchNorm2d(num_features,eps,momentum)
    def forward(self, inputs: Tensor) -> Tensor:
        out = nn.functional.relu(self.bnorm1(inputs))
        return out
class standard_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        self.con1 = nn.Conv2d(first_num_filters, filters, kernel_size=3, stride=1, padding=1, bias=False)
        self.bnorm1 = nn.BatchNorm2d(filters)
        self.con2 = nn.Conv2d(filters, filters, kernel_size=3,stride=1, padding=1, bias=False)
        self.bnorm2 = nn.BatchNorm2d(filters)
        self.shortcut = nn.Sequential()

        if projection_shortcut is not None:
            self.con1 = nn.Conv2d(first_num_filters, filters, kernel_size=3, stride=strides, padding=1, bias=False)
            self.bnorm1 = nn.BatchNorm2d(filters)
            self.con2 = nn.Conv2d(filters, filters, kernel_size=3,stride=1, padding=1, bias=False)
            self.bnorm2 = nn.BatchNorm2d(filters)
            self.shortcut = nn.Sequential(
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        # print(type(inputs))
        out = nn.functional.relu(self.bnorm1(self.con1(inputs)))
        out = nn.functional.relu(self.bnorm2(self.con2(out)))
...
class bottleneck_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        # Args given above.
        self.bnorm1 = nn.BatchNorm2d(filters)
        self.con1 = nn.Conv2d(filters, int(filters/4), kernel_size=1, stride=1, bias=False)
        self.bnorm2 = nn.BatchNorm2d(int(filters/4))
        self.con2 = nn.Conv2d(int(filters/4), int(filters/4), kernel_size=3,stride=1, padding=1, bias=False)
        self.bnorm3 = nn.BatchNorm2d(int(filters/4))
        self.con3 = nn.Conv2d(int(filters/4), filters, kernel_size=1,stride=1, bias=False)
        self.shortcut = nn.Sequential()

        if projection_shortcut is not None:
            self.bnorm1 = nn.BatchNorm2d(first_num_filters)
            self.con1 = nn.Conv2d(first_num_filters, int(filters/4), kernel_size=1, stride=strides, bias=False)
            self.bnorm2 = nn.BatchNorm2d(int(filters/4))
            self.con2 = nn.Conv2d(int(filters/4), int(filters/4), kernel_size=3,stride=1, padding=1, bias=False)
    ...
    # since it performs a 1x1 convolution.
        out = nn.functional.relu(self.bnorm1(inputs))
        out = self.con1(out)
        out = nn.functional.relu(self.bnorm2(out))
        out = self.con2(out)
        out = nn.functional.relu(self.bnorm3(out))
        out = self.con3(out)
        out += self.shortcut(inputs)
        return out
